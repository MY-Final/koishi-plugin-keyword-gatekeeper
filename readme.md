...
- 🖼️ **图片智能识别**：自动识别QQ表情包和图片链接，避免误判
- 🔒 **管理员豁免**：智能检测群管理员和群主，自动跳过关键词检测
- 🛡️ **智能白名单**：内置常见媒体域名白名单，减少误报
...
启用自动处罚机制后，插件会根据用户触发关键词或发送非白名单URL的次数，自动升级处罚等级：

1. **第一次触发**：警告提示并撤回消息
2. **第二次触发**：禁言指定时长（可自定义，默认60秒）
3. **第三次及以上触发**：
   - 禁言时间会随违规次数递增：禁言时长 = 第二次违规时长 × (违规次数-1)
   - 例如：若第二次违规禁言60秒，则第三次违规禁言120秒，第四次违规禁言180秒...
   - 如果达到最大违规次数：踢出群聊或长时间禁言（可配置）

> **重要**：自动处罚机制对关键词和URL检测均有效。当启用该机制后，系统会自动累计用户的违规次数，无论是触发关键词还是发送非白名单URL，都会增加计数并升级处罚等级。

系统会记录每个用户的触发次数，并支持按时间重置（默认24小时）。您可以在配置中调整第二次违规的禁言时长和最大违规次数，以便自定义处罚的强度。
...
- `kw.preset.imported [群组ID]` - 查看当前群组或指定群组已导入的预设包
...
- `kw.group.unimport-preset <预设包名称>` - 从当前群组移除预设关键词包

## 📝 版本更新

### 🆕 v0.1.3 更新内容

- 🔍 **全面增强URL检测**：彻底改进URL检测引擎，使用多重正则表达式精准识别各种URL格式
- 🛠️ **修复特殊URL问题**：解决带前缀字符的URL和纯域名无法识别的问题
- 🖥️ **IP地址支持增强**：优化对IP地址形式URL的识别能力
- 📊 **调试系统完善**：增加详细日志信息和智能错误处理机制
- ⚡ **检测精度大幅提升**：通过多层次检测策略显著提高URL检测的准确性和鲁棒性

### 🆕 v0.1.2 更新内容

- 添加了管理员检测功能，跳过对管理员的关键词检测
- 优化日志系统和处罚机制，确保升级处罚正常工作
- 大幅改进URL检测功能，识别并排除QQ表情包和图片链接

### 🆕 v0.1.1
- 🔄 **预设包管理优化**：添加了从群组中卸载预设包的功能
- 📋 **命令增强**：新增了 `kw.group.unimport-preset` 命令，支持从群组中移除指定预设包
- 🧹 **关键词清理**：允许更灵活地管理群组中的关键词，可以一次性移除整个预设包的词汇
- 🔧 **维护便利性**：方便管理员在不同场景下调整群组的关键词配置

### v0.1.0
- 🔄 **关键词检测优化**：修复了全局关键词和群组关键词检测冲突的问题
- 🔍 **双重检测机制**：现在全局和群组关键词会同时检测，确保所有违规消息都能被正确处理
- 🚫 **增强过滤可靠性**：无论是全局设置还是群组特定设置中的关键词都能有效触发
- 🛡️ **更稳定的管理体验**：确保无论群组配置如何，所有违规内容都会被拦截处理

### v0.0.9
- 🔧 **关键词检测修复**：修复了全局关键词检测和撤回功能的问题
- 📊 **日志优化**：减少了不必要的日志输出，提高了性能
- 🔄 **代码优化**：改进了消息处理流程，使检测更加准确高效
...
```
> kw.group.unimport-preset common
成功从群组中移除预设包 "common"，共移除 3 个关键词。
...
```
> kw.group.imported-presets
群组 123456789 已导入的预设包：
...
群组共有 12 个关键词，其中一些关键词可能来自多个预设包。
使用 kw.group.unimport-preset <名称> 来移除特定预设包。
```
...
### 🆕 v0.1.4 更新内容
- 🔍 **全面增强URL检测**：添加了分享卡片和媒体消息的检测，避免误判；优化了URL检测逻辑，提高了准确性；添加了更多常见域名到白名单；增强了消息预处理，更准确地分离和处理消息中的媒体内容；完善了自动处罚机制，确保URL检测与关键词检测共享相同的处罚升级逻辑，实现URL禁言时长随违规次数递增
...
- 修复了全局关键词和群组关键词检测冲突的问题
- 现在全局和群组关键词会同时检测，确保所有违规消息都能被正确处理
- 优化了关键词检测机制，提高了系统稳定性和可靠性
- 确保无论群组配置如何，所有违规内容都会被拦截处理
...
- 添加了从群组中卸载预设包的功能
- 新增 `kw.group.unimport-preset` 命令，可以从群组中移除指定预设包中的关键词
- 支持更灵活地管理群组的关键词配置
- 优化了预设包管理体验
...
关键词守门员插件内置调试模式，可用于排查问题和观察详细的工作流程。默认情况下调试模式处于关闭状态，可通过以下命令控制：
...
> kw.debug -d
调试模式已禁用，将只显示重要的警告和错误信息
```

调试模式启用后，控制台会输出更详细的日志信息，包括关键词匹配过程、消息处理细节等。这对于排查问题和了解插件工作原理很有帮助。
请注意，在生产环境中建议关闭调试模式，以减少日志量和提高性能。
